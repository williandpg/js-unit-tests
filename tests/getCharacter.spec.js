...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    const objetoAryaCorreto = {
      name: 'Arya Stark',
      class: 'Rogue',
      phrases: ['Not today', 'A girl has no name.']
    };
    expect(getCharacter('Arya')).toEqual(objetoAryaCorreto);
    // 3. Teste se a função retorna o objeto correto para o parâmetro 'Brienne',
    const objetoBrienneCorreto = {
      name: 'Brienne Tarth',
      class: 'Knight',
      phrases: [ 'Im No Lady, Your Grace.', 'I, Brienne Of Tarth, Sentence You To Die.' ]
    };
    expect(getCharacter('Brienne')).toEqual(objetoBrienneCorreto);
    // 4. Teste se a função retorna o objeto correto para o parâmetro 'Melissandre',
    const objetoMelissandreCorreto = {
      name: 'Melissandre',
      class: 'Necromancer',
      phrases: [ 'Death By Fire Is The Purest Death.', 'For The Night Is Dark And Full Of Terrors.' ]
      };
    expect(getCharacter('Melissandre')).toEqual(objetoMelissandreCorreto);
    ...
    expect(getCharacter('Stark')).toEqual(undefined);
  });
});
